 ...
 * int64 为基础的仿真时间。
 * 精度由缩放指数来决定，这个参数是全局性的，每个实例都共享，指数从-18-0，比如设置为-6时候，表示的的微秒。
 * 从double转换而来的使用double类型的缩放，从整数转换而来的使用int64，
 * 以前是消除了int64  double转换，现在的编译器是优化了表达式为一个普通的整数操作，不需要做乘法运算。
 * 
 * 专门为DOUBLE和CPAR提供了一组转换函数以及数学函数，也为整型提供了模板类。
 * @ingroup SimTime
 */
class SIM_API SimTime
{
  private:
    ...
    // 如前面说的，这几个参数都是静态全局变量 setScaleExp(int e)中设置
    static int scaleexp;     // scale exponent in the range -18..0  缩放指数
    static int64_t dscale;   // 10^-scaleexp, that is 1 or 1000 or 1000000...    // 用来做除法的一个10幂数的
    static double fscale;    // 10^-scaleexp, that is 1 or 1000 or 1000000...    // 和dscale是一样的。

    // 这个值是前面2个值的倒数，存这个值，主要是因为乘法肯定比除法快。
    ...
    static bool checkmul;    // when true, multiplications are checked for integer overflow，是否检查乘法溢出
