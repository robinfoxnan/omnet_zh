...
/*
int cModule::handleForeignCmd(char * cmd, char* param)
{
    return -1;
}
*/
...
bool cModule::isSimple() const
{
    ...
    bool ret1 = dynamic_cast<const cSimpleModule *>(this) != 0; //nullptr;
    std::string name = this->getFullPath();
    //std::cout << "测试模块是否是simpleModule:"  << "=" <<ret1 << std::endl;
    return ret1;
}
...
// void Qtenv::finishSimulation()调用进入cSimulation::callFinish()
// 最终执行到这，还是作为网络图的根节点
...
// void Qtenv::callRefreshDisplay()中调用此处
// 从qtenv的systemModule*作为入口，开始递归更新
...
/*
int cModule::handleUserCmd(char * cmd, char* param)
{
      std::cout << "cModule 处理外部导调命令的基本函数，在子类中重写\n";
       return -1;
}
*/
