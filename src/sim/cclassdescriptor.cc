...
cClassDescriptor *cClassDescriptor::getDescriptorFor(const cObject *object)
{
    ...
    // 这段原理是这样的：因为注册描述符时候，构造函数指定了类名以及父类的名字，
    char * tmpclassName = (char *)object->getClassName();
    cClassDescriptor *desc = cClassDescriptor::getDescriptorFor(tmpclassName);
    std::cout << tmpclassName << " --> ";
    if (desc)
    {
        std::cout << desc->getClassName() << std::endl;
        return desc;
     } 

    // 如果找不到合适的描述符，就试着去找一个最早的基类
