...
// 根据所有的子模块，计算一个平均对角线长，并根据这个值推算一个占用的合适大小；
void ForceDirectedGraphLayouter::calculateExpectedMeasures()
{
    // 之前添加的BODY，都添加到了这个embedding里面了，同时添加到了全局映射表中
    ...
    // 这段算法就是：将各个模块的对角线长相加，将各个模块的面积相加
    for (auto body : bodies) {
        if (!dynamic_cast<WallBody *>(body))     // 只要不是边界BODY
        {
            double length = body->getSize().getDiagonalLength();   // 对角线长度，
            count++;
            maxBodyLength = std::max(maxBodyLength, length);       // 计算各个模块最大值
    ...
    // 这里是计算是否有特别大的模型，比如说对角线比 2倍平均值 还要大
    ...
    // 保证1个最小值20，然后根据个数多少 + 1个弹性值
    double minEdgeLength = 20 + 2400 / (20 + bodies.size());
    // 如果有相对较大的图形，则需要再加上平均长度
...
// NED中没有指定任何位置信息的节点，会有一个默认的图标大小，确定的长和宽，位置是不确定的；
...
// 在界面上已经指定了具体位置的： @display("p=135,103");
// 会使用此函数添加一个节点，
...
// 这个函数最终实现了所有无位置的NODE的设置，
void ForceDirectedGraphLayouter::execute()
{
    if (hasMovableNode || hasAnchoredNode) 
    {
        calculateExpectedMeasures();   // 计算所有的子模块放在一起大概需要占用多少空间合适
        setParameters();               // 暂时没读懂
