...
// 这个函数在Qtenv::doRunSimulation()中的循环里，不停的调用
// 参数：onlyHold是真的时候表示需要保持动画，

bool DisplayUpdateController::animateUntilNextEvent(bool onlyHold)
{
    // 如果没有任何消息了，当前仿真应该是结束了
...
double DisplayUpdateController::getAnimationSpeed() const
{
    ...
    double msgAnimSpeed = msgAnim->getAnimationSpeed(); // 遍历管理的所有动画，确定一个最小速度
...
bool DisplayUpdateController::renderUntilNextEvent(bool onlyHold)
{
    ...
    // in animationSpeed，其实是按照帧率，计算采样间隔
...
// 将动画界面等时间推进到内核仿真时间
// 动画速度其实是相对仿真时间的缩放倍率（默认是1.0），所以这里用除法，计算播放动画用的时长；
void DisplayUpdateController::advanceToSimTime(TimeTriplet &triplet, SimTime simTarget)
{
    ASSERT(simTarget >= triplet.simTime);

    SimTime simDt = simTarget - triplet.simTime;  // 计算差值时间还要多少

    double animSpeed = getAnimationSpeed();       // 计算动画速度，这个值相当于： 针对仿真时间的倍速

    double animDt = (animSpeed == 0) ? 0 : simDt.dbl() / animSpeed;  // 得到可以动画允许播放的时长

    triplet.simTime += simDt;
    triplet.animationTime += animDt;

    ASSERT(triplet.simTime == simTarget);  // 最后为啥还确认一下？
}
...
void DisplayUpdateController::renderFrame(bool record)
{
    ...
    qtenv->updateSimtimeDisplay();   // 底部状态栏的消息显示
    qtenv->updateStatusDisplay();    // 这里面包含了上一句代码，重复了，是不是可以删除上一句？
    ...
    qtenv->callRefreshDisplay();    // 刷新网络图界面
    qtenv->refreshInspectors();     // 刷新几个其他的界面

    QApplication::processEvents();  // 移交控制权，防止界面死锁
