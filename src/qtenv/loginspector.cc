...
{
    toolBar->addAction(QIcon(":/tools/copy"), "拷贝选择文本到剪切板 (Ctrl+C)\n按 Ctrl+Shift+C 可以包含格式",
                       textWidget, SLOT(copySelectionUnformatted()))->setShortcut(Qt::ControlModifier + Qt::Key_C);
    toolBar->addAction(QIcon(":/tools/find"), "查找字符串(Ctrl+F)",
    ...
    if (isTopLevel()) // looks like we don't need this in embedded mode, for whatever reason...
        toolBar->addAction(QIcon(":/tools/save"), "保存窗口内容到文件",
                           this, SLOT(saveContent()));
    toolBar->addAction(QIcon(":/tools/filter"), "过滤窗口内容(Ctrl+H)",
                       this, SLOT(onFilterButton()))->setShortcut(Qt::ControlModifier + Qt::Key_H);
    configureMessagePrinterAction
            = toolBar->addAction(QIcon(":/tools/config"), "配置消息显示",
    ...
    toMessagesModeAction->setText("显示消息/数据包");
