...
void Inspector::createCopyDropdownMenu()
{
    ...
    action = copyDropdownMenu->addAction("拷贝全路径", getQtenv(), SLOT(utilitiesSubMenu()));
    action->setData(QVariant::fromValue(CopyActionData{object, COPY_FULLPATH}));
    action = copyDropdownMenu->addAction("拷贝名字", getQtenv(), SLOT(utilitiesSubMenu()));
    action->setData(QVariant::fromValue(CopyActionData{object, COPY_FULLNAME}));
    action = copyDropdownMenu->addAction("拷贝类名", getQtenv(), SLOT(utilitiesSubMenu()));
