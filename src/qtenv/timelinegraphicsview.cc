...
{
    // 返回了一个double类型的时间值，并对这个double 取一个log10，得到此时间的指数级别，然后根据指数范围算宽度
...
{
    QVector<int> labelRowLastX;  
    // right edge of last label written in each row, with [0] being the bottom and [numRows-1] the top line
...
// 将消息在指定的位置画一个椭圆，并添加附加消息指针，以及TOOlTIP
...
void TimeLineGraphicsView::rebuildScene()
{
    ...
    // draw zero delta tray，画[]括号，刻度线高度为5+5
    ...
    // draw ticks，大的刻度间隔，如果间隔太小，就无法写字
    int tickSpacing = (maxExponentX-minExponentX) / (maxExponent-minExponent);
    bool reduceTickLabels = tickSpacing < maxTickLabelSize.width()+10;  
    // if there's not enough space, we only write out 1ns, 1us, 1ms, 1s,1000s etc.
    
    // 大的刻度，按照log10计算，刚好是按照平均分配的原则确定的
    for (int exp = std::max(minExponent-1, simtime_t::getScaleExp()); exp <= maxExponent; exp++) {
        if (exp >= minExponent) {
            // tick
            int x = getXForExponent(exp);  // 计算某个指数级别的刻度
        ...
        // 小刻度按照log10计算，就会有些曲率变化，这里不能取1，因为log10(1) == 0
        if (drawMinorTicks)
            for (int i = 2; i <= 9; i++) {
                int x = getXForTimeDelta(SimTime(i, (SimTimeUnit)exp));
                scene()->addLine(x, axisY-3, x, axisY);
            }
    }
