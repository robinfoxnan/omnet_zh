...
// 2个模块，获取共同的祖先，用了1个2维循环比较，效率不太高，最差的情况时间负责度M * N
// 后续可以用2个1维循环来实现，
...
// 记录从SRC到DST的路径的经过的节点数组
// 这里的方法是从SRC向上找到共同的祖先，和上一段代码一样，然后下降
...
//----------------------------------------------------------------------
// 在模块转换为图元的时候，需要将所有的${param}换成具体的值，这里的函数需要最后一个参数为true;
