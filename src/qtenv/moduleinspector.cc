...
ModuleInspector::ModuleInspector(QWidget *parent, bool isTopLevel, InspectorFactory *f) : Inspector(parent, isTopLevel, f)
{
    ...
    showModuleNamesAction = new QAction("显示模块名字", this);
    ...
    showArrowheadsAction = new QAction("显示箭头", this);
    ...
    increaseIconSizeAction = new QAction("放大图标", this);
    ...
    decreaseIconSizeAction = new QAction("缩小图标", this);
...
QToolBar *ModuleInspector::createToolbar(bool isTopLevel)
{
    ...
    if (isTopLevel)
        ...
        // general
        goBackAction = toolbar->addAction(QIcon(":/tools/back"), "后退", this, SLOT(goBack()));
        goForwardAction = toolbar->addAction(QIcon(":/tools/forward"), "前进", this, SLOT(goForward()));
        goUpAction = toolbar->addAction(QIcon(":/tools/parent"), "转到父模块", this, SLOT(inspectParent()));
    }
    toolbar->addSeparator();

    toolbar->addAction(QIcon(":/tools/mrun"), "运行至下一事件", this, SLOT(runUntil()));
    QAction *action = toolbar->addAction(QIcon(":/tools/mfast"), "快速运行至下一事件 (Ctrl+F4)", this, SLOT(fastRunUntil()));
    action->setShortcut(Qt::CTRL | Qt::Key_F4);
    toolbar->addAction(getQtenv()->getMainWindow()->getStopAction());
    toolbar->addSeparator();

    // canvas-specfic
    action = toolbar->addAction(QIcon(":/tools/redraw"), "重置布局", this, SLOT(relayout()));
    action->setShortcut(Qt::CTRL | Qt::Key_R);
    canvasRelayoutAction = action;
    action = toolbar->addAction(QIcon(":/tools/zoomin"), "缩小", this, SLOT(zoomIn()));
    action->setShortcut(Qt::CTRL | Qt::Key_Plus);
    canvasZoomInAction = action;
    action = toolbar->addAction(QIcon(":/tools/zoomout"), "放大", this, SLOT(zoomOut()));
    action->setShortcut(Qt::CTRL + Qt::Key_Minus);
    canvasZoomOutAction = action;

    // osg-specific
    action = toolbar->addAction(QIcon(":/tools/reset"), "重置视图");
    resetOsgViewAction = action;
    toolbar->addSeparator();

    // mode switching
    action = toolbar->addAction(QIcon(":/tools/3dscene"), "3D 场景", this, SLOT(switchToOsgView()));
    action->setCheckable(true);
    switchToOsgViewAction = action;

    action = toolbar->addAction(QIcon(":/tools/modulegraphics"), "模型", this, SLOT(switchToCanvasView()));
    action->setCheckable(true);
...
// qtevn::newNetwork函数调用到这里，设置网络图，将图形设置到界面上
void ModuleInspector::doSetObject(cObject *obj)
{
    ...
    if (object) {
        ...
        try {
            ...
            QMessageBox::warning(this, QString("错误"), QString("显示网络图出现错误: ") + e.what());
        }
...
void ModuleInspector::relayout()
{
    if (getQtenv()->getSimulationState() != Qtenv::SIM_ERROR) {
        ...
        getQtenv()->getMainWindow()->busy("重布局中...");
...
void ModuleInspector::createContextMenu(const std::vector<cObject *>& objects, const QPoint& globalPos)
{
    ...
    menu->addSeparator();
    menu->addAction("显示/隐藏 画布图层...", this, SLOT(showCanvasLayersDialog()));
    ...
    menu->addSeparator();
    menu->addAction("布局设置...", this, SLOT(runPreferencesDialog()))
            ->setData(QVariant::fromValue(TAB_LAYOUTING));
    menu->addAction("动画设置...", this, SLOT(runPreferencesDialog()))
            ->setData(QVariant::fromValue(TAB_ANIMATION));
    menu->addAction("动画过滤...", this, SLOT(runPreferencesDialog()))
            ->setData(QVariant::fromValue(TAB_FILTERING));

    menu->addSeparator();
    menu->addAction("导出为图片...", canvasViewer, SLOT(exportToImage()));
    menu->addAction("导出为PDF...", canvasViewer, SLOT(exportToPdf()));
    menu->addAction("打印...", canvasViewer, SLOT(print()));
...
void ModuleInspector::showCanvasLayersDialog()
{
    ...
    if (!canvasRenderer->hasCanvas()) {
        QMessageBox::warning(this, tr("警告"), tr("这个模块没有创建默认的画布"),
                QMessageBox::Ok);
        return;
    }
