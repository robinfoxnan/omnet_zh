...
void ModuleCanvasViewer::drawSubmodule(cModule *submod)
{
    ...
    // 设置子模块的位置，这个函数是使用了布局器来计算某个模块的位置
...
void ModuleCanvasViewer::redrawEnclosingModule()
{
    ...
    // 这里是对复合的模型确定其大小；根据背景指定大大小，或者子模型大小；
...
void ModuleCanvasViewer::redraw()
{
    ...
    refreshLayout();   // 需要确定每个模块都指定了位置，否则需要使用自动布局工具设置一个位置；
    redrawModules();   // 先画子模块，画当前复合模块，再画连接；因为子模块图元画完才知道大小，
    redrawFigures();
    refreshSubmodules();
    refreshConnections();
