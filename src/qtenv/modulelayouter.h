...
// 在讲解NED时候，提及了network中涉及几种不同的默认布局方式
class QTENV_API ModuleLayouter : public QObject {
    ...
    std::unordered_map<std::string, int> layoutSeeds;                    // 模块全名到种子的映射，如果种子用光了，就设置为1
    // stores the layouted positions of submodules in their parents
    std::unordered_map<const cModule *, QPointF> modulePositions;         // 子模块到位置的映射
