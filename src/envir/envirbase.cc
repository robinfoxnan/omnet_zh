...
// 这里的第一个参数ID不是数字，在宏中是指针，后面所有参数用来生成类，由这个指针来保管，
// 后面查找的时候，通过？来查找
    ...
    try {
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (foldersLoaded.find(folder) == foldersLoaded.end()) {
                if (opt->verbose)
                    std::cout << "加载 NED 文件的目录： " << folder ;
                int count = getSimulation()->loadNedSourceFolder(folder);
                if (opt->verbose)
                    std::cout << " 共计： " << count << endl;
...
/* 
    if (!network)  // robin added  inet.tutorials.queueing.BufferTutorialStep
    {
        inifilePackage = "inet.tutorials.queueing";
         network = cModuleType::find((inifilePackage + "." + networkname).c_str());
    }
    */
    if (!network)
