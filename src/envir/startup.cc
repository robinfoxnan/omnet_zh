...
int setupUserInterface(int argc, char *argv[])
{
    ...
    try {
        ...
        if (verbose) {
            std::cout << " 网络协议仿真器  (C) 2001-2020 Huaru.com." << endl;
        ...
        // to instantiate.
        // 文件名通过-f指定，或者直接参数指定，或者不指定使用默认的名字
        ...
        if (appName.empty())
            appName = config->getAsString(CFGID_USER_INTERFACE);   // user-interface
        ...
        //
        if (verbose)
            std::cout << "准备使用界面库： " << appReg->getName() << "..." << endl << endl;
